...
void reduceBright(){
  ...
  if(menuOpenFlag == 0){
    if(flag == 0){
      ...
      IrSender.sendNEC(sAddress, EEPROM.readInt(0), sRepeats);  // 메뉴
      Serial.println(EEPROM.readInt(0), HEX);
      delay(500);
      IrSender.sendNEC(sAddress, EEPROM.readInt(28), sRepeats); // 순서 1
      Serial.println(EEPROM.readInt(28), HEX);
      delay(500);
      IrSender.sendNEC(sAddress, EEPROM.readInt(32), sRepeats); // 순서 2
      Serial.println(EEPROM.readInt(32), HEX);
      delay(500);
      IrSender.sendNEC(sAddress, EEPROM.readInt(36), sRepeats); // 순서 3
      Serial.println(EEPROM.readInt(36), HEX);
      delay(500);
      //IrSender.sendNEC(sAddress, EEPROM.readInt(20), sRepeats);
  
      for(int i = 0; i< 16; i++){
        IrSender.sendNEC(sAddress, EEPROM.readInt(8), sRepeats);
        Serial.println(EEPROM.readInt(8), HEX);
        delay(100);
      }
    }
  }
...
void increaseBright(){
  if (menuOpenFlag == 1){   // 값 측정-> detectFlag , 메뉴 열림 확인->  menuOpenFlag 
    ...
    for(int i = 0; i< 16; i++){
      IrSender.sendNEC(sAddress, EEPROM.readInt(4), sRepeats);
      Serial.println(EEPROM.readInt(4), HEX);
      delay(100);
    }
    IrSender.sendNEC(sAddress, EEPROM.readInt(20), sRepeats); // 메뉴 종료
    Serial.println(EEPROM.readInt(20), HEX);
    detectFlag = 0;
    menuOpenFlag = 0;
    RGB_ON_OFF(1, 0);
  }
